...
then
	echo -e "-Don't Ignore libtestdisk.a For Git"
	git update-index --no-assume-unchanged ${LIB_DIR}/libtestdisk.a > /dev/null 2>&1
else
	echo -e "-Ignore libtestdisk.a For Git"
	git update-index --assume-unchanged ${LIB_DIR}/libtestdisk.a > /dev/null 2>&1
fi
...
then
    ...
    then
        echo -e "-libtestdisk Exsist\n-Wanna Rebuild？\n-【Y/N Default:N】\c"
        read IsLibTestdiskReCompile
        if [ ${IsLibTestdiskReCompile} == "Y" ] > /dev/null 2>&1
        then
            # 环境清理
            if [ -f "${LIB_DIR}/libtestdisk.a" ]
            then
                rm -rf ${LIB_DIR}/libtestdisk.a
            fi
            echo -e "-Cleaning Temporal Files\c"
            make clean > /dev/null 2>&1
            make distclean > /dev/null 2>&1
...
echo -e ""
if [ -f "${TESTDISK_DIR}/src/libtestdisk.a" ]
then
    mv ${TESTDISK_DIR}/src/libtestdisk.a ${LIB_DIR}/libtestdisk.a
    echo -e "-libtestdisk Building Complete\c"
else
    echo -e "-Do Nothing\c"
fi
